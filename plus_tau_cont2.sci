...
N=70;// number of steps (graphic drawn with dash line)
for k=1:N
    Omega_plus=list();
    Omega_minus=list();
    for i=1:m
  ...
  dif=zeros(1,m);
  for i=1:op
      dif(Omega_plus(i))=(r_t(Omega_plus(i))-r(Omega_plus(i)))*s;
  end
  r_t=r_t-dif';
  dif=zeros(1,m);
  for i=1:om
      dif(Omega_minus(i))=(p_t(Omega_minus(i))-p(Omega_minus(i)))*h;
  end
  p_t=p_t-dif';
